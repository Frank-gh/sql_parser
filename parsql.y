...
CSQLStmt g_cSqlStmt;
bool g_bFinishField;
string g_sWhereFldName;
string g_sWhereFldValue;
FLDTYPE g_iWhereFldType;
int g_iWhereCmp;
vector<WHERE_REL> g_vWhereRel;
%}
...
opt_where: /* nil */ 
   | WHERE expr { 
     emit("WHERE"); 
     //emit("g_vWhereRel = %d m_iConditionCount = %d ",g_vWhereRel.size(),g_cSqlStmt.m_iConditionCount);
     if (g_vWhereRel.size() != g_cSqlStmt.m_iConditionCount - 1) 
        g_cSqlStmt.m_vSqlStmtError = true;
     g_cSqlStmt.AddWhereRel(g_vWhereRel);
    };
...
select_expr_list: select_expr { $$ = 1; }
    ...
    | '*' { emit("SELECTALL"); g_cSqlStmt.m_bAllFlds = true; $$ = 1; }
    ;
...
expr: NAME          { emit("NAME %s", $1); if(!g_bFinishField) g_cSqlStmt.m_vFlds.push_back($1); else g_sWhereFldName = $1;  ($1); }
   | USERVAR         { emit("USERVAR %s", $1); free($1); }
   | NAME '.' NAME { emit("FIELDNAME %s.%s", $1, $3); free($1); free($3); }
   | STRING        { emit("STRING %s", $1); g_sWhereFldValue = $1; g_iWhereFldType = _STRING; free($1); }
   | INTNUM        { emit("NUMBER %d", $1); g_sWhereFldValue = num2str($1);}
   | APPROXNUM     { emit("FLOAT %g", $1); g_sWhereFldValue = float2str($1); }
   | BOOL          { emit("BOOL %d", $1); }
   ;
...
expr: expr '+' expr { emit("ADD"); }
   ...
   | expr ANDOP expr { emit("AND"); g_vWhereRel.push_back(_AND);}
   | expr OR expr { emit("OR"); g_vWhereRel.push_back(_OR);}
   | expr XOR expr { emit("XOR"); }
   | expr COMPARISON expr { emit("CMP %d", $2); if (4 == $2) g_cSqlStmt.AddWhereCondition(g_sWhereFldName,g_iWhereFldType,g_sWhereFldValue);}
...
void clear()
{
  g_bFinishField = false;
  g_sWhereFldName = "";
  g_sWhereFldValue = "";
  g_iWhereFldType = _NONE_FLD;
  g_iWhereCmp = _NONE_REL;
  g_vWhereRel.clear();
}
...
{
  ofstream ofile;
  ofile.open("./.input.sql");
  ofile << sql << endl;
  ofile.close();
  clear();

  extern FILE *yyin;
  if((yyin = fopen("./.input.sql", "r")) == NULL) {
    g_cSqlStmt.m_vSqlStmtError = true;    
    printf("ERROR: %s\n",strerror(errno));
    return -1;
  }
  // 记录历史执行过的SQL
  FILE *sql_his;
  if((sql_his = fopen("./his.sql", "a+")) == NULL) {
    return -1;
  }
  fseek(sql_his, 0, SEEK_END);
  char sql_str[1024];
  memset(sql_str,0x0,sizeof(sql_str));
  sprintf(sql_str,"%s\n",sql.c_str());
  fwrite(sql_str, strlen(sql_str), 1, sql_his);
  fclose(sql_his);
  // end
